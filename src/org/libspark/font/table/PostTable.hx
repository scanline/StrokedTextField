...
class PostTable
{
		private var macGlyphName:Array<String> = [
			".notdef",      // 0
			"null",         // 1
			"CR",           // 2
			"space",        // 3
	...
	private var glyphNameIndex:Array<Int>;
	private var psGlyphName:Array<Dynamic>;
	...
	public function new(de:DirectoryEntry, byte_ar:ByteArray)
	{
		...
		italicAngle = byte_ar.readInt();
		underlinePosition = byte_ar.readShort();
		underlineThickness = byte_ar.readShort();
		isFixedPitch = byte_ar.readInt();
		minMemType42 = byte_ar.readInt();
		maxMemType42 = byte_ar.readInt();
		minMemType1 = byte_ar.readInt();
		maxMemType1 = byte_ar.readInt();
		...
		if (version == 0x00020000)
		{
			...
			{
				glyphNameIndex.push(byte_ar.readUnsignedShort());
			}
			...
			if (h>257)
			{
				...
				for (j in 0...h)
				{
					var len:Int = byte_ar.readUnsignedByte();
